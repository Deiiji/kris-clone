...
class LLWorldSitOnGround : public view_listener_t
{
	bool handleEvent(const LLSD& userdata)
	{
		if (isAgentAvatarValid()&& !gAgentAvatarp->isSitting())  // KL 2.0.2 merge change
		{
			gAgent.setControlFlags(AGENT_CONTROL_SIT_ON_GROUND);
		} else
		{
...
class LLWorldCheckSitOnGround : public view_listener_t
{
	...
	{
		bool sitting = FALSE;
		if (isAgentAvatarValid()) // KL 2.0.2 merge change
		{
			sitting = gAgentAvatarp->isSitting(); // Kl 2.0.2 merge change
		}
		return sitting;
	}
};
