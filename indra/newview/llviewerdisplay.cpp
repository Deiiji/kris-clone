...
void display(BOOL rebuild, F32 zoom_factor, int subfield, BOOL for_snapshot)
{
	...
	if(gSavedSettings.getBOOL("StereoMode")==TRUE) // 0 = off, 1 = Anaglyph via mask 2 = active/clone-shutter mode
	{
	     ...
	     // Draw to the right buffer (right eye)
	   	 // setup colour mask for anaglyph
		 // ok this is how the mask is set currently, the previous method is encapsulated in this.
	...
	// KL 3D This is the location for the Second Eye routine
	// why because both left n right are supposed to be in here and second eye exists AFTER LLSpatialGroup::sNoDelete = FALSE is closed.
	if (gSavedSettings.getBOOL("StereoMode")==TRUE)
	{
