...
class LLFloaterWindLightNotecardCreatedCallback : public LLInventoryCallback
{
public:
...
void LLFloaterWindLight::onSaveNotecard()
{
	...
	{
		LLNotificationsUtil::add("WLSaveNotecardAlert", LLSD(), LLSD(), boost::bind(&LLFloaterWindLight::saveNotecardCallback, this, _1, _2));
	}
	else
	{
		...
		LLPointer<LLFloaterWindLightNotecardCreatedCallback> cb = new LLFloaterWindLightNotecardCreatedCallback();
...
void LLFloaterWindLightNotecardCreatedCallback::fire(const LLUUID& inv_item)
{
	LLWLParamManager * param_mgr = LLWLParamManager::instance();
