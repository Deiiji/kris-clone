...
void LLPipeline::allocateScreenBuffer(U32 resX, U32 resY)
{
	...
	{ //disable multisampling of render targets where ATI is involved
	   // KL allow ATI driver tweakers the ability to experiment
	    ...
	    {
	    samples = gGLManager.getNumFBOFSAASamples(gSavedSettings.getU32("RenderFSAASamples")); // Fix that GCC definition error
	    }
	    else
	    {
		samples = 0;
...
void LLPipeline::renderBloom(BOOL for_snapshot, F32 zoom_factor, int subfield)
{
	...
	gGL.setColorMask(true, true); // Render bloom this cannot be masked for 3d here.
	glClearColor(0,0,0,0);
	...
	if (LLPipeline::sRenderDeferred)
	{
		...
		else if (!dof_enabled || LLToolMgr::getInstance()->inBuildMode() || !gSavedSettings.getBOOL("RenderUseDOF"))
...
{	
	if (LLPipeline::sWaterReflections && assertInitialized() && LLDrawPoolWater::sNeedsReflectionUpdate)
	{
		...
		{	//generate planar reflection map
			...
			gGL.setColorMask(mask[0],mask[1],mask[2],false); // KL hopefully mWaterRef.clear dont obliterate the mask.... :/
		...
		if (last_update)
		{
			...
			if (!LLPipeline::sUnderWaterRender || LLDrawPoolWater::sNeedsReflectionUpdate)
			{
				...
				GLboolean mask[4];
                glGetBooleanv(GL_COLOR_WRITEMASK,mask);
				gGL.setColorMask(mask[0],mask[1],mask[2],false); // KL grabbing the mask it 'should' still be the same.
...
{
   if (!gSavedSettings.getBOOL("RenderHoverGlowEnable")) //  KL 
	{
		return;
	}
