...
void LLPipeline::renderBloom(BOOL for_snapshot, F32 zoom_factor, int subfield)
{
	...
	if (LLPipeline::sRenderDeferred && renderDOF && !LLViewerCamera::getInstance()->cameraUnderWater())
	{
		...
		if (LLToolMgr::getInstance()->inBuildMode() || !gSavedSettings.getBOOL("RenderUseDOF")) // S21 version
...
void LLPipeline::generateSunShadow(LLCamera& camera)
{
	...
	for (S32 j = 0; j < 4; j++)
	{
		...
		//put all points in wpf in quadrant 0, relative to center of min/max
