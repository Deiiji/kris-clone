...
public:
	...
	struct PostProcessTweaks : public LLSD {
		...
		inline LLSD & useAnaglyphVisionShader() {
			return (*this) ["enable_anaglyph_vision"];
		}

		inline F32 OffsetS() const {
			return F32((*this) ["offset_s"].asReal());
		}

		inline F32 OffsetT() const {
			return F32((*this) ["offset_t"].asReal());
		}

		inline LLSD & useToonShader() {
			return (* this)["enable_toon_shader"];
		}

		inline F32 Silhouette() const {
			return F32((*this) ["silhouette_threshold"].asReal());
		}
