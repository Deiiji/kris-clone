...
if [ -n "$LL_RUN_ERR" ]; then
	...
	if [ "$LL_RUN_ERR" = "runerr" ]; then
		...
		if [ "`uname -m`" = "x86_64" ]; then
			echo
			cat << EOFMARKER
You are running the Kirstens Viewer on a x86_64 platform.  The
most common problems when launching the Viewer may be solved by
installing your Linux distribution's 32-bit compatibility packages.
