    ...
    def package_finish(self):
        ...
        if self.default_channel():
            if self.default_grid():
                ...
                # beta grid viewer
                installer_file = "Kirstens_S21_%(version_dashes)s_(%(grid_caps)s) Setup.exe"
                grid_vars_template = """
                OutFile "%(installer_file)s"
                !define INSTFLAGS "%(flags)s"
            ...
            installer_file = "Kirstens_21_%(version_dashes)s_%(channel_oneword)s Setup.exe"
            grid_vars_template = """
            OutFile "%(installer_file)s"
            !define INSTFLAGS "%(flags)s"
...
class LinuxManifest(ViewerManifest):
    def construct(self):
        ...
        if self.prefix("linux_tools", dst=""):
            ...
            self.path("wrapper.sh","Runme-kirstens-S21")
...
class Linux_i686Manifest(LinuxManifest):
    def construct(self):
        ...
        if self.prefix("../packages/lib/release", dst="lib"):
            ...
            self.path("libaprutil-1.so.0.3.12")
	    ...
	    self.path("libminizip.so")
            ...
            self.path("libdirectfb-1.4.so.9.0.1")
            self.path("libfusion-1.4.so.9.0.1")
            self.path("libdirect-1.4.so.9.0.1")
